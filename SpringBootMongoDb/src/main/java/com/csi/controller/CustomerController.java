...
public class CustomerController {
    ...
    public ResponseEntity<Customer> updateData(@PathVariable int custId, @RequestBody Customer customer) {
        Customer customer1 = customerServiceImpl.getDataById(custId).orElseThrow(()-> new RecordNotFoundException("Cust Id not reg"));
    ...
    public ResponseEntity<String> deleteDataById(@PathVariable int custId) {
        customerServiceImpl.delteDataById(custId);
        return ResponseEntity.ok("Cust data get delete..");
    }
